  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      if (!tokenRes.ok) {
        ...
        if (err.fallback) {
          setState((s) => ({ ...s, status: "error", error: "Realtime API unavailable. Using fallback mode." }));
          return;
        }
      ...
      if (!token) {
        setState((s) => ({ ...s, status: "error", error: "No ephemeral token received. Voice may not be available." }));
        return;
      }
