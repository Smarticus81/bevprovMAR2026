              ...
              <div className="relative z-10 flex items-center gap-6 sm:gap-8">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2.5 mb-1.5">
                    <span className="text-[11px] font-semibold text-[#C9A96E] uppercase tracking-[0.15em]">Featured</span>
                ...
                {filteredItems.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-[11px] text-white/25 mb-2" data-testid={`text-agent-type-${item.id}`}>
                        {AGENT_TYPE_LABELS[item.type]}
                      </p>
                      <p className="text-xs text-white/35 leading-relaxed line-clamp-2 mb-4 min-h-[2.5rem]">
                        {item.description}
                      </p>

                      <div className="flex items-center justify-between pt-3 border-t border-white/[0.06]">
                        <span className="text-[11px] text-white/20 font-medium">{toolCount} tools</span>
                        <button
            ...
            >
              <div className="sticky top-0 bg-[#0a0a0a] z-10 pt-3 pb-2 px-6">
                ...
                <button
                  ...
                  onClick={() => setSelectedAgent(null)}
                  className="absolute top-4 right-4 p-2 hover:bg-white/[0.06] transition-colors"
                >
  ...
  return (
    <div>
          ...
          <div className="flex items-center gap-2 mt-3">
            <span
              ...
              className={`inline-flex items-center px-2.5 py-0.5 text-[11px] font-semibold border ${
                isActive ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-white/[0.04] text-white/35 border-white/[0.06]"
              }`}
            >
              {isActive ? "Active" : item.isTemplate ? "Available" : item.status}
            </span>
            <span className="inline-flex items-center px-2.5 py-0.5 text-[11px] font-medium bg-white/[0.03] text-white/25 border border-white/[0.06]">
              {tools.length} tools
            </span>
          </div>
        </div>
      ...
      <div className="mb-6 pb-6 border-b border-white/[0.06]">
        <h3 className="text-xs font-semibold text-[#C9A96E]/70 uppercase tracking-[0.15em] mb-3">About</h3>
        <p className="text-sm text-white/50 leading-relaxed" data-testid="text-detail-description">
          {item.description}
        </p>
      </div>

      <div className="mb-6 pb-6 border-b border-white/[0.06]">
        <h3 className="text-xs font-semibold text-[#C9A96E]/70 uppercase tracking-[0.15em] mb-3">
          Capabilities
        </h3>
        <div className="space-y-4" data-testid="list-tools">
          {Object.entries(toolsByCategory).map(([category, categoryTools]) => (
            <div key={category}>
              <p className="text-[11px] font-medium text-white/20 uppercase tracking-wider mb-2">{category}</p>
                ...
                {categoryTools.map((tool) => (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <p className="text-xs font-medium text-white/60">{tool.name.replace(/_/g, " ")}</p>
                      <p className="text-[11px] text-white/25 mt-0.5 leading-relaxed">{tool.description}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div className="mb-8">
        <h3 className="text-xs font-semibold text-[#C9A96E]/70 uppercase tracking-[0.15em] mb-3">Voice Configuration</h3>
        <div className="bg-white/[0.02] p-4 space-y-3 border border-white/[0.04]" data-testid="section-voice-config">
          {[
            ...
            <div key={label} className="flex justify-between items-center text-sm">
              ...
              <span className="text-white/60 font-medium text-xs bg-white/[0.03] px-2.5 py-1 border border-white/[0.06]">{value}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="sticky bottom-0 bg-[#0a0a0a] pt-4 pb-2 border-t border-white/[0.06]">
        {!item.isTemplate ? (
          <button
            ...
            onClick={() => onOpen(item)}
            className="w-full py-3.5 bg-[#C9A96E] text-black font-semibold text-sm hover:bg-[#C9A96E]/90 transition-all duration-200"
          >
            Configure Agent
          </button>
        ) : (
          <button
            ...
            disabled={activating}
            className="w-full py-3.5 bg-[#C9A96E] text-black font-semibold text-sm hover:bg-[#C9A96E]/90 transition-all duration-200 disabled:opacity-50"
          >
